	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				if (Integer.parseInt(message) <= ticket.getTicket()) {
					ticket.setTicket(ticket.getTicket() - Integer.parseInt(message));
					// Print all the message to all clients, Group chat
					for (int i = 0; i < socketList.size(); i++) {
						writer = new PrintWriter(socketList.get(i).getOutputStream());
						writer.println("Number of available tickets: " + ticket.getTicket());
						writer.flush();
					}
				} else {
					for (int i = 0; i < socketList.size(); i++) {
						writer = new PrintWriter(socketList.get(i).getOutputStream());
						writer.println("Insufficient number of available tickets.");
						writer.flush();
					}
				}

...
Ext
		.define(
				'KanenOnlineShopping.controller.productController',
				{
					...
					clearCart : function(button, e, eOpts) {
						...
						if (cartStore.count() > 0) {
							...
							Ext.Msg.confirm('Clear Cart',
									'Remove all items from your cart?',
									function(btn) {
										if (btn == 'yes') {
											Ext.getStore('cartStore')
													.removeAll();
					...
					onOrderItemsGridPanelItemDblClick : function(dataview,
							record, item, index, e, eOpts) {

						// debugger;
						var orderItemsGridPanel = Ext
								.getCmp('orderItemsGridPanel');
						...
						// productDescription:
						// responseData.items[i].productDescription,
						...
						// },
						// failure: function(){
						// Ext.MessageBox.alert('Login failed', 'Unable to
						// connect to server,
						// please try again.');
						// }
						// });
					...
					onBtnMainRegisterClick : function(button, e, eOpts) {
						// var windowReg =
						// Ext.create('KanenOnlineShopping.view.RegisterWindow',
						// {});
					...
					onBtnRegRegisterClick : function(button, e, eOpts) {
						...
						var userName = frmRegister.query('#txtRegUsername')[0]
								.getValue();
						var userPassword = frmRegister.query('#txtRegPassword')[0]
								.getValue();
						var confirmPassword = frmRegister
								.query('#txtRegConfirmPassword')[0].getValue();
						var fullName = frmRegister.query('#txtRegFullname')[0]
								.getValue();
						var email = frmRegister.query('#txtRegEmail')[0]
								.getValue();
						var address = frmRegister.query('#txtRegAddress')[0]
								.getValue();
						var contactNumber = frmRegister
								.query('#txtRegContactNumber')[0].getValue();
						...
						if (frmRegister.isValid()) {
							if (userPassword === confirmPassword) {
								...
								Ext.Ajax
										.request({
											...
											success : function(response) {
												var responseText = Ext
														.decode(response.responseText);
												if (responseText.data.errormsg
														.indexOf('none') > -1) {
													Ext.MessageBox
															.alert(
																	'Registation',
																	'Account Created, You are now registered!');
													this
								...
								Ext.MessageBox.alert('Invalid confirmPassword',
										'Passwords do not match.');
							}
						} else {
							Ext.MessageBox.alert('Missing fields',
						...
						if (cartStore.count() > 0) {
							...
							cartStore
									.each(function(record) {
										totalCost = totalCost
												+ (record.data.productPrice * record.data.quantity);
									});
					...
					onLaunch : function() {
						...
						this.registerWindow = Ext.create(
								'KanenOnlineShopping.view.RegisterWindow', {});
						this.registerWindow.hide();

						this.loginWindow = Ext.create(
								'KanenOnlineShopping.view.LoginWindow', {});
						this.loginWindow.hide();

						this.cartWindow = Ext.create(
								'KanenOnlineShopping.view.CartWindow', {});
						this.cartWindow.hide();

						this.orderHistoryContainer = Ext
								.getCmp('orderHistoryContainer');
					...
					loadProducts : function(productStore) {
						Ext.Ajax
								.request({
									...
									success : function(response) {
										...
										var productPanel = Ext
												.getCmp('productPanel');
										productStore
												.each(function(record) {
													var mystr = '';

													var itemContainer = Ext
															.create(
																	'KanenOnlineShopping.view.itemContainer',
																	{
													...
													itemNameLabel.text = record.data.productName
															+ record.data.productDescription;

													// var itemDescriptionLabel
													// =
													// itemContainer.items.items[2];
						 ...
						 * productStore.each(function(record){ var mystr =''; //
						 * debugger;
						 * 
						 * var itemContainer =
						 * Ext.create('KanenOnlineShopping.view.itemContainer', {
						 * 
						 * });
						 * 
						 * itemContainer.productId = record.data.productId;
						 * itemContainer.productName = record.data.productName;
						 * itemContainer.productDescription =
						 * record.data.productDescription;
						 * itemContainer.productPrice =
						 * record.data.productPrice;
						 * itemContainer.productStockQuantity =
						 * record.data.productStockQuantity;
						 * itemContainer.productImagePath =
						 * record.data.productImagePath;
						 * 
						 * var itemImage = itemContainer.items.items[0];
						 * itemImage.src = record.data.productImagePath;
						 * 
						 ...
						 * itemContainer.items.items[2];
						 * //itemDescriptionLabel.text =
						 * record.data.productDescription; var itemBtn =
						 * itemContainer.items.items[2]; // debugger;
						 * itemBtn.tooltip=record.data.productDescription;
						 * 
					...
					addItemToCart : function(productId, productName,
							productDescription, productPrice,
							productStockQuantity, productImagePath) {
						// alert(productName);
						var cartStore = Ext.getStore('cartStore');
					...
					resetRegistrationFields : function() {
						...
						frmRegister.query('#txtRegConfirmPassword')[0]
								.setValue('');
						...
						frmRegister.query('#txtRegContactNumber')[0]
								.setValue('');
					...
					checkIfUserExist : function(userName, userStore) {
						...
						if (userNameArray.length > 0) {
							Ext.MessageBox.alert('User exist',

...
CREATE TABLE WISH_LIST(
   WISHLIST_ID            NUMBER(15)          NOT NULL PRIMARY KEY
  ,USER_ID       		      NUMBER(15)          NOT NULL
  ,PRODUCT_ID             NUMBER(15)          NOT NULL
  ,PRODUCT_NAME           VARCHAR(100)        NOT NULL
  ,PRODUCT_DESCRIPTION    VARCHAR(220)        NOT NULL
...
CREATE SEQUENCE WISH_LIST_ID_SEQ START WITH 7000001 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER WISH_LIST_ID_SEQ
BEFORE INSERT ON WISH_LIST
FOR EACH ROW
BEGIN
IF :new.WISH_LIST_ID IS NULL THEN
SELECT WISH_LIST_ID_SEQ.nextval INTO :new.WISH_LIST_ID FROM DUAL;
END IF;
END;


--CREATE TABLE PRODUCT_COMMENT_ASSN
CREATE TABLE PRODUCT_COMMENT_ASSN (
     PRODUCT_COMMENT_ASSN_ID NUMBER(15) NOT NULL PRIMARY KEY
    ,PRODUCT_COMMENT VARCHAR(200)
    ,USER_ID NUMBER(15) NOT NULL
    ,PRODUCT_ID NUMBER(15) NOT NULL
    ,FOREIGN KEY(USER_ID) REFERENCES USERS (USER_ID)
...
/*
 SEQUENCE AND TRIGGER FOR PRODUCT_COMMENT_ASSN TABLE
*/
CREATE SEQUENCE PRODUCT_COMMENT_ASSN_ID_SEQ START WITH 6000000001 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER PRODUCT_COMMENT_ASSN_ID_TRIG
BEFORE INSERT ON PRODUCT_COMMENT_ASSN
FOR EACH ROW
BEGIN
IF :new.PRODUCT_COMMENT_ASSN_ID IS NULL THEN
SELECT PRODUCT_COMMENT_ASSN_ID_SEQ.nextval INTO :new.PRODUCT_COMMENT_ASSN_ID FROM DUAL;
END IF;
END;

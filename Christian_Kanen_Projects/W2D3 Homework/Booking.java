	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				ticket.setTicket(ticket.getTicket() - t);
				int i = 0;
				for (i = 0; i < socketList.size(); i++) {
					writer = new PrintWriter(socketList.get(i).getOutputStream());
					writer.println("Client " + count + " has booked " + t + " ticket/s. Tickets remaining: " + ticket.getTicket());
					writer.flush();
				}

				System.out.println("Client #" + i + " booked " + t + " tickets. Tickets Left: " + this.ticket.getTicket());
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
